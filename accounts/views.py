	...
	if request.method=='POST':
		...
		if password1==password2:
			if User.objects.filter(username=username).exists():
				...
				user=User.objects.create_user(username, password=password1, email=email, mobile=mobile, gender=gender, dob=dob, balance= 0)
				user.save()
	...
	if request.method=='POST':
		...
		if password1==password2:
			if User.objects.filter(username=username).exists():
				...
				user=User.objects.create_user(username, password=password1, email=email, mobile=mobile, gender=gender, dob=dob, bank=bank, branch=branch, balance= 0, is_staff = True)
				user.save();
				messages.success(request,"Account Created Successfully")
				return redirect('home')
		else:
